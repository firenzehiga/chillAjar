            ...
            const fetchCourse = async () => {
                try {
                    ...
                    if (response.data.jadwal_kursus) {
                        const initial = response.data.jadwal_kursus.map(
                            (jadwal) => ({
                                id: jadwal.id,
            ...
            if (response.status === 200 || response.status === 201) {
                ...
                if (isEditMode) {
                    ...
                    if (!newCourseId) {
                        ...
                        console.log(
                            "Fetch Course Response:",
                            fetchCourseResponse.data
                        );
                        const latestCourse = fetchCourseResponse.data
                            .filter(
                                (course) =>
                                    course.namaKursus === formData.namaKursus
                            )
                            .sort(
                                (a, b) =>
                ...
                <form onSubmit={handleSubmit}>
                    ...
                    <div className="mb-4">
                        ...
                        <button
                            ...
                            <Plus className="w-5 h-5 inline mr-2" /> Add
                            Schedule
                        </button>
                    </div>
